...
- File search uses the local Spotlight index and is off by default. Filename search can be enabled independently; indexed file-content search, hidden files, Trash, and system locations are separate opt-ins. Keyestro never requests Full Disk Access automatically.
...
Configuration exports exclude Keychain values, clipboard payloads, passwords, secret arguments, private environment values, and managed script bodies. They also exclude local privacy, capability, and safety choices: usage learning; clipboard monitoring and pause state; Quick Paste and its sensitive-content consent; file search and each expanded file scope; and the local sleep-confirmation acknowledgement. A schema-v1 export from an older Keyestro version can still be inspected, but those keys are ignored on import so a file cannot authorize a capability or weaken a local consent choice on another Mac. Clipboard retention and excluded-application preferences remain portable settings; they are applied only after the quick-link database transaction commits and cannot enable clipboard capture by themselves. The permission-protected import journal contains portable pre-import settings and registration metadata solely for interrupted-import recovery; it is local to this installation, excludes local capability grants, and is removed or marked complete before settings-dependent services resume.

The Privacy settings surface provides per-type clipboard deletion and an exact local-data deletion workflow. Clearing caches affects only Keyestro's bundle-owned cache directory.
